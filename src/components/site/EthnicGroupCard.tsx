...
export default function EthnicGroupCard({ group }: { group: EthnicGroup }) {
  return (
    <Link href={`/ethnic/${group.slug}`} className="group relative overflow-hidden rounded-barna aspect-square bg-barna-cream shadow-sm hover:shadow-lg transition-all duration-300">
      {group.image ? (
        <Image src={group.image} alt={group.display_name} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 50vw, 25vw" />
      ) : (
      ...
      <div className="absolute bottom-0 inset-x-0 p-4">
        <h3 className="text-white font-bold text-sm md:text-base">{group.display_name}</h3>
