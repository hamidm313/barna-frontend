...
const heroImages = ['/images/logo/barna-logo.jpg','/images/hero/travel-report.svg','/images/hero/workshop.svg','/images/hero/accessories.svg'];
  ...
  return (
    <section className="relative min-h-[82vh] flex items-center overflow-hidden bg-[radial-gradient(circle_at_top_left,#fff7df,transparent_34%),linear-gradient(135deg,#fffaf0,#f8f2e7_45%,#ffffff)]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid lg:grid-cols-[1fr_520px] gap-14 items-center">
        <div>
          <p className="inline-flex rounded-full bg-white/80 border border-primary-200 px-4 py-2 text-primary-700 font-semibold text-sm mb-5 shadow-sm">{t('home.heroBadge')}</p>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-barna-dark leading-tight mb-6">
            {t('home.heroTitleA')}<br />{t('home.heroTitleB')} <span className="text-primary-600">{t('home.heroTitleHighlight')}</span>
          </h1>
          <p className="text-barna-gray text-lg md:text-xl leading-9 mb-8 max-w-2xl">{t('home.heroSubtitle')}</p>
          <div className="flex flex-wrap gap-4">
            <Link href="/clothing" className="btn-primary">{t('home.reserveCta')}</Link>
            <Link href="/community" className="btn-outline">{t('home.communityCta')}</Link>
            <Link href="/about" className="btn-outline">{t('home.aboutCta')}</Link>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-10 max-w-xl">
            {[["۸+", t('home.statEthnic')], ["۳", t('home.statPaths')], ["۱۰۰٪", t('home.statReady')]].map(([n, l]) => (
