...
import { getComments, updateCommentStatus, deleteComment } from '@/lib/api/comments';
...
export default function CommentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['comments', page, filter],
    queryFn: () => getComments({ page: page + 1, limit: 10, status: filter === 'all' ? undefined : filter }),
  });
  const updateMut = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => updateCommentStatus(id, status),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['comments'] }); toast.success('بروز شد'); },
  });
  const deleteMut = useMutation({
    mutationFn: (id: number) => deleteComment(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['comments'] }); toast.success('حذف شد'); },
  });
  ...
  const columns: Column<Comment>[] = [
    ...
    { key: 'content', label: 'متن', render: (r) => <Typography sx={{ fontFamily: 'Vazirmatn, sans-serif', fontSize: '0.85rem', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', maxWidth: 300 }}>{r.content}</Typography> },
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h5" sx={{ fontFamily: 'Vazirmatn, sans-serif', fontWeight: 700, color: '#1E2A4A' }}>مدیریت نظرات</Typography>
      ...
      <DataTable columns={columns} rows={data?.data ?? []} keyField="id" loading={isLoading} total={data?.pagination?.total} page={page} rowsPerPage={10} onPageChange={setPage} emptyMessage="نظری وجود ندارد" />
    </Box>
  );
}
